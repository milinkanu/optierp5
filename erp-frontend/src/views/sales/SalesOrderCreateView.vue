<template>
  <div class="page">
    <PageHeader :title="isEdit ? 'Edit Sales Order' : 'New Sales Order'" :subtitle="isEdit ? 'Update details of your confirmed sales order.' : 'Create a new sales order to confirm customer request.'">
      <template #actions>
        <Button variant="secondary" @click="$router.push({ name: 'SalesOrders' })">Cancel</Button>
        <Button :loading="saving" @click="saveSalesOrder">{{ isEdit ? 'Save Changes' : 'Create Sales Order' }}</Button>
      </template>
    </PageHeader>
      ...
      <Card class="card">
        <div class="form-grid">
          ...
          <label>
            ...
            <input v-model="form.sales_order_number" class="input" placeholder="SO-2026-00001" :disabled="isEdit" />
          </label>
          ...
          <label>
            <div class="label">Sales Order Date*</div>
            <input v-model="form.sales_order_date" type="date" class="input" />
          </label>

          <label>
            <div class="label">Expected Shipment Date (optional)</div>
            <input v-model="form.expected_shipment_date" type="date" class="input" />
          </label>

          <label>
            <div class="label">Payment Terms (optional)</div>
          ...
          <label class="span2">
            ...
            <input v-model="form.subject" class="input" placeholder="Order Summary / Title" />
          </label>
          ...
          <label class="span2">
            ...
            <textarea v-model="form.terms_and_conditions" class="input input-textarea" placeholder="Standard order terms, delivery policy..." rows="3"></textarea>
          </label>
        </div>
      </Card>
      ...
      <Card class="card">
        ...
        <div class="totals-layout">
          <div class="notes-hint">
            <p>Calculated dynamic GST, TDS, and TCS are based on local tax compliance standards.</p>
          </div>
...
const isEdit = computed(() => !!route.params.salesOrderId)

const today = new Date()
const iso = (d) => d.toISOString().slice(0, 10)

const form = reactive({
  sales_order_number: `SO-${today.getFullYear()}-${Math.floor(10000 + Math.random() * 90000)}`,
  sales_order_date: iso(today),
...
onMounted(async () => {
  ...
  if (isEdit.value) {
    try {
      const existing = await salesOrdersApi.get(route.params.salesOrderId)
      if (existing) {
        form.sales_order_number = existing.sales_order_number
        form.sales_order_date = existing.sales_order_date
        form.expected_shipment_date = existing.expected_shipment_date || ''
        ...
        form.payment_terms = existing.payment_terms || 'Due on Receipt'
        ...
        form.quote_id = existing.quote_id || null
...
const saveSalesOrder = async () => {
  ...
  try {
    ...
    if (isEdit.value) {
      await salesOrdersApi.update(route.params.salesOrderId, payload)
      toast.success('Sales order updated successfully')
    } else {
      const created = await salesOrdersApi.create(payload)
      toast.success(`Created Sales Order ${created.sales_order_number}`)
    }
    router.push({ name: 'SalesOrders' })
  } catch (e) {
    const msg = e?.response?.data?.detail || e?.message || 'Failed to save sales order'
